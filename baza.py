...
def zmien_stan(id_produktu, nowy_stan):
    if nowy_stan < 0:
        return
    if nowy_stan == 0:
        supabase.table("produkty").update({"liczba": 0}).eq("id", id_produktu).execute()
    else:
...
def usun_kategorie(kategoria_id):
    ...
    if produkty_w_kat:
        st.error("❌ Najpierw usuń produkty z tej kategorii")
        return
    ...
    if not produkty:
        ...
        for p in produkty:
            with st.container(border=True):
                ...
                col_qty, col_minus, col_plus, col_zero, col_del = st.columns([2,1,1,2,2])
